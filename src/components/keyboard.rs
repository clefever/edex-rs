...
pub fn Keyboard(layout: ReadSignal<KbLayout>) -> Element {
    let row_numbers = layout.read().row_numbers.clone();
    let row_1 = layout.read().row_1.clone();
    let row_2 = layout.read().row_2.clone();
    let row_3 = layout.read().row_3.clone();
    ...
    rsx! {
        ...
        section { id: "keyboard",
            ...
            KeyboardRow { id: String::from("row_2"), keys: row_2 }
            KeyboardRow { id: String::from("row_3"), keys: row_3 }
            KeyboardRow { id: String::from("row_space"), keys: row_space }
        }
    }
}

#[component]
fn KeyboardRow(id: String, keys: Vec<Key>) -> Element {
    rsx! {
        div {
            class: "keyboard_row", id: "{id}",
            for key in keys {
                KeyboardKey {
                    name: key.name,
                    cmd: key.cmd,
